...
class FacturacionExpressController extends BaseController {
	...
	public function consultaCedula($cedula,$nombre){
		
		#buscamos la cedula de qualitas y la retornamos
	    $cedulaQualitas = CedulaQualitas::where('CQ_folioautorizacion',$cedula)->get();
	    if(count($cedulaQualitas)==0){
	    	return 0;
	    }
	    ...
	    if($contCedulas>1){
	    	...
	    	$contPaciente =strlen($nombreCedula);	    	
	    	$nombreTruncado = substr($nombreExpediente,0,$contPaciente);
	    	if($nombreTruncado==$nombreCedula){
	    		return $cedulaQualitas[0];
	    	}else{
	    		return 2;
	    	}
	    }	    
